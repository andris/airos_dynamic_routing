...
function setOperationMode(status) {
	...
	if (wireless.cwmmode == 1) {
		if (wireless.apmac == "00:00:00:00:00:00")
			wd_str = l10n_get('Auto') + ' 20 / 40';
		else
			wd_str = l10n_get('Auto') + ' ' + wireless.chwidth;
	}
...
function update_ack(status) {
	...
	if ((wireless.polling.enabled == 0 || wireless.polling.noack == 0) && status.wlan.enabled) {
		...
		var dist_km = (wireless.distance / 1000.00).toFixed(1);
		var dist_mi = (wireless.distance / 1609.344).toFixed(1);
...
function secsToCountdown(seconds) {
	...
	if (days > 0) {
		result =  "" + days + " " + ((days == 1) ? l10n_get('day') : l10n_get('days')) + " " + result;
	}
	return result;
}
...
function update_misc(status, qos, uptime, ccq, date_time) {
	...
	var uptime_str = secsToCountdown(status.host.uptime);
...
function update_wanlan(lan, wan) {
	...
	if (lan.status.length == 2) {
		$('#lan1_cable').text(l10n_get(lan.status[0]));
		$('#lan2_cable').text(l10n_get(lan.status[1]));
		return 0;
	} else {
		if (wan.status.length == 1) {
			if (lan.status.length == 1) {
				$('#lan1_cable').text(l10n_get(wan.status[0]));
				$('#lan2_cable').text(l10n_get(lan.status[0]));
				return;
			} else {
				$('#lan_cable').text(l10n_get(wan.status[0]));
				return;
			}
		}
	}
	$('#lan_cable').text(l10n_get(lan.status[0]));
}
