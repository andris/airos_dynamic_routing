...
td { padding$tchain-left: 20px; padding-right: 20px; padding-top: 5px }
div.errmsg {
color: red;
font-weight: bold;
...
function checkDone() {
	...
	jQuery.getJSON("testdone.cgi?"+(new Date().getTime()), function(d) {
		if (d.done) {
       		...
       		check_timer = setTimeout("checkDone()", 1000 * <?if(strlen($eth_do)==0){>5<?}else{>1<?}>);
	});
	return false;
}
...
<td><input id="eth_do" name="eth_do" type="checkbox" <?if(strlen($eth_do)!=0) { echo "checked";}>"></td>
</tr>
...
<? if ($REQUEST_METHOD=="POST" && $action=="test") {
    ...
    fputs($fp, "for i in `grep eth /proc/net/dev | cut -f 1 -d \":\"`; do ethtool \${i}; done > /tmp/test-eth.dat\n");
...
if (fileinode("/tmp/test-started") != -1) {
	...
	@unlink("/tmp/test-started");
	@unlink("/tmp/test-done");
	if (fileinode("/tmp/test-flash.dat") != -1) {
	...
	while ($i < $test_links) {
		...
		if (fileinode("/tmp/test-eth.dat") != -1) {
			$array = File("/tmp/test-eth.dat");
			
			$i = 0;
			while  ($i < count($array)) {
				if (ereg("Settings for",$array[$i])) {
					$link = "";
					$duplex = "";
					$speed = "";
					$max_speed = " 100";
					...
					$found = 0;
					while  ($i < count($array) && $found == 0) {
						if (ereg("Speed:",$array[$i])) {
							$speed = $array[$i];
						}
						elseif (ereg("Duplex:",$array[$i])) {
							$duplex = $array[$i];
						}
						elseif (ereg("Link detected:",$array[$i])) {
							$link = $array[$i];
							$found = 1;
							$i--;
						}
						elseif (ereg("1000baseT",$array[$i])) {
							$max_speed = " 1000";
						}
						elseif (ereg("PHYAD:",$array[$i])) {
							$phyaddr = $array[$i];
						}
						$i++;
					}
					
					if ((($phycount == 1) && ereg("PHYAD: 4",$phyaddr)) || ($phycount > 1)) {
						if ($link != "") {
							$fail = 0;
							...
							if (ereg("PHYAD: 4",$phyaddr)) {
								$eth_port = "PRIMARY";
							}
							else {
								$portnum = strtok($phyaddr," ");
								$portnum = strtok(" ");
								$eth_port = "SECONDARY($portnum)";
							}
